...
// --- Frequency・VFO ---
unsigned long FREQ = 14050000;
unsigned long FREQ_OLD = FREQ;
unsigned long long FREQ_ULL = 1405000000ULL;
...
#define SCOPE_SENSITIVITY 14.0f    // Amplitude gain of the scope waveform (the higher the value, the higher the waveform rises)
#define SCOPE_OFFSET      3.1f     // Raising the scope's noise floor (to reveal invisible micro-noise)
...
// RF frequency display for the main VFO readout
// Round down to the nearest 10 Hz and display in 10 Hz increments
// Example: 14,000,123Hz -> "14.000.12"
//          14,074,987Hz -> "14.074.98"
static String fmtFreqFull(unsigned long f_hz){
  char buf[20];
  ...
  // Hz range (0–999 Hz)
  unsigned long hz  = f_hz % 1000UL;

  // Rounded down to the nearest 10 Hz
  // Example: 987Hz -> 98
  //          123Hz -> 12
  unsigned long hz10 = hz / 10UL;

  // The Hz section displays two digits
  snprintf(buf, sizeof(buf), "%lu.%03lu.%02lu", mhz, khz, hz10);
...
void setup() {
  ...
  u8g2.clearBuffer();
  u8g2.drawStr(34, 8, "14MHz CW TRX v1.0");
